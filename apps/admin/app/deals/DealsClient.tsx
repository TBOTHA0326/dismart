...
export default function DealsClient({ profile, initialDeals, branches }: Props) {
  ...
  return (
    <AdminShell role={profile.role}>
      ...
      <Table
        headers={["Title", "Branch", "Source", "Status", "Order", ""]}
        ...
        rows={deals.map((deal) => [
          ...
          <span key="source" className="capitalize text-xs text-gray-500">{deal.source.replace("_", " ")}</span>,
      ...
      <DealModal open={modalOpen} onClose={() => setModalOpen(false)} onSaved={handleSaved} branches={branches} deal={editing} />
    </AdminShell>
  );
}
